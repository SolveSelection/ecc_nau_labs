    ...
    public partial class Form1 : Form
    {
        ...
        private void bOutFile_Click(object sender, EventArgs e)
        {
            ...
            if (saveFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                textBoxOutFile.Text = fileOUTpath;
            }
        }

        private void bStart_Click(object sender, EventArgs e)
        {
            byte[] inArr = File.ReadAllBytes(textBoxInFile.Text);
            byte[] outArr = myEncoding(inArr);
            File.WriteAllBytes(textBoxOutFile.Text, outArr);

            // out file size to byte
            long length = new FileInfo(textBoxOutFile.Text).Length;
            label_size_out.Text = length.ToString();
        ...
        {
            long length = new FileInfo(textBoxInFile.Text).Length;
            // in file size from byte to mb
            label_size_in.Text = ((length / 1024f) / 1024f).ToString();
        }
